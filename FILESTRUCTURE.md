...
* C++ source files:
  * `include/cpp-pointer/cpp-pointer.hpp` is the main
    C++ header that declares the interface of your library.
  * `app/cpp-pointer_app.cpp` is an executable that uses the library.
    This can e.g. be used to provide a command line interface for your project.
  * `tests/cpp-pointer_t.cpp` contains the unit tests for the library.
